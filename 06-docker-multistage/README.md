...
enrollment number, and the screenshot/output evidence).

This README is the working notes: what a multi-stage build is, the Dockerfile that was
cloned and built, the full log, and a size comparison that turned out more interesting
than expected.
...
├── Dockerfile           an EXTRA multi-stage example, in Go (see the last section)
├── app/
│   ├── main.go
│   └── go.mod
└── screenshots/
    ├── app-running-8080.png     the cloned app in the browser on port 8080
    └── 11-multistage-8080.png   docker ps + curl in the terminal
```

## The Dockerfile that was cloned

Task 1 says to clone the repository containing the multi-stage Dockerfile. That is the
class repo, **[MineshShaw/devops-heros](https://github.com/MineshShaw/devops-heros)**,
under `session6-7-docker/multi-stage-dockerfile/`:
...
It is an Express app that responds with
`<h1>Hello World from Docker Multi-Stage Build!</h1>` and listens on **port 3000 inside
the container**, so it is published with `-p 8080:3000` to meet the "running on port
8080" requirement.

```bash
git clone https://github.com/MineshShaw/devops-heros.git
...
with its own filesystem, and a later stage copies only what it needs with
`COPY --from=<stage>`. Everything else — compilers, SDKs, caches, source, dev
dependencies — is discarded when the build ends.

```
┌─ Stage 1: builder ──────────────┐      ┌─ Stage 2: production ──────┐
│ FROM node:24-alpine AS builder  │      │ FROM node:24-alpine        │
│ npm install  (ALL deps)         │ ───► │ COPY --from=builder ...    │
│ full source tree                │ only │ npm install --omit=dev     │
│ discarded at the end            │ this │ server.js only             │
└─────────────────────────────────┘      └────────────────────────────┘
```

## The result — and an honest surprise

Building the **same app single-stage** for comparison:

| Build | Image | Size |
|---|---|---|
| Single-stage (`FROM node:24-alpine`, `npm install`, `COPY . .`) | `singlestage-hello` | **249 MB** |
| **Multi-stage** (builder → production) | `multistage-hello` | **243 MB** |

Only **6 MB** saved. That is worth understanding rather than glossing over: both stages
use the **same base image**, and the production stage still runs `npm install --omit=dev`,
so the only thing actually dropped is the dev-dependency tree and the untracked files
`COPY . .` would have pulled in. For an interpreted language whose "build" produces no
separate artifact, multi-stage mostly buys you *hygiene* — a controlled file list and no
dev dependencies — not a dramatic size cut.

`docker history multistage-hello` confirms what the final image contains: the Alpine +
Node base, `npm install --omit=dev` (9.45 MB), and a 12.3 kB `COPY` for `server.js`.
The builder stage does not appear at all.

**Multi-stage pays off dramatically when the build tool is not needed at runtime.** The
extra example in this folder is the same idea in Go:

| Build | Image | Size |
|---|---|---|
| The Go build toolchain alone (`golang:1.22-alpine`) | base image | **344 MB** |
| **Multi-stage Go app** (`golang` builder → `alpine` runtime) | `go-multistage` | **20.2 MB** |

A **17× difference**, because the compiler produces a static binary and nothing else has
to ship. Same principle for Java (`jdk` → `jre`) and React (`node` → `nginx`), both used
in [`../05-docker-hello-world/`](../05-docker-hello-world/).

## Full build, run and verify log

```console

############################################################
#  TASK 1 - Clone the repository containing the multi-stage Dockerfile
############################################################
...
############################################################
#  Run a container from the image on port 8080
############################################################
...
############################################################
#  Why multi-stage matters - build the SAME app single-stage and compare
############################################################
...
############################################################
#  EXTRA - the same idea taken further, with a compiled language
############################################################
...
[`../05-docker-hello-world/`](../05-docker-hello-world/) — Node.js, Python, Java, Apache,
React and Nginx, each with its own Dockerfile, all verified in a browser. Two of those
(Java and React) are themselves multi-stage builds.
